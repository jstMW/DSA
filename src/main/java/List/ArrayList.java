...
public classs ArrayList<T> implements AbstractArrayList<T> {
    ...
    public void add(int index, T element) throws IllegalStateException {
        if (size==data.length) {throw new IllegalStateException("the ArrayList is full!");}
    ...
    //when you get back try to write the test for this code since it is actually wrong and will generate a lot of errro so try to practice teh SQA and TEst Diven development adn also Ci Cd!!!!
    //also note that you are soooooo back and need to be prepared for testQQQ and more important the assignment!!!
}
